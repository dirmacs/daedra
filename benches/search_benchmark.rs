...
fn create_test_response(result_count: usize) -> SearchResponse {
    let results: Vec<SearchResult> = (0..result_count)
        .map(|i| SearchResult {
            ...
            description: format!("This is test result number {} with some description text", i),
...
fn bench_cache_operations(c: &mut Criterion) {
    ...
    for size in [1, 10, 50].iter() {
        group.bench_with_input(
            BenchmarkId::new("cache_set", size),
            size,
            |b, &size| {
        ...
        group.bench_with_input(
            BenchmarkId::new("cache_get", size),
            size,
            |b, &size| {
                let cache = SearchCache::with_defaults();
                let response = create_test_response(size);
...
fn bench_serialization(c: &mut Criterion) {
    ...
    for size in [1, 10, 50].iter() {
        ...
        group.bench_with_input(
            ...
            |b, json| {
                b.iter(|| {
                    serde_json::from_str::<SearchResponse>(black_box(json)).unwrap()
                });
            },
        );
    }
